...
const Navbar = () => {
  ...
  return (
    <div className='bg-secondary py-4 px-8 flex items-center justify-between'>
      <div className='text-lg font-semibold cursor-pointer' onClick={() => router.push("/")}>
        Blogify
      </div>
      <div className='flex items-center gap-6'>
        ...
        {!isLoggedIn ? (
          <>
            <Link href={"/login"} className='cursor-pointer text-sm hover:underline'>Login</Link>
            <Link href={"/register"} className='cursor-pointer text-sm hover:underline'>Register</Link>
          </>
        ) : (
         <>
         <div>Hi {userState.username}</div>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will log you out of your account. You can always log in again.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
